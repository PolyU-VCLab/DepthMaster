...
def solve_optimal_focal_shift(uv: np.ndarray, xyz: np.ndarray):
    ...
    # 过滤掉 NaN / Inf / z<=0 的退化点，避免 fn 在 shift=0 时产生非有限残差
    valid = (
        np.isfinite(z)
    ...
    if valid.sum() < 8:
        # 有效点过少，无法稳健求解，返回退化解（shift=0, focal=1）
        return np.float32(0.0), np.float32(1.0)
    uv, xy, z = uv[valid], xy[valid], z[valid]
    ...
    def fn(uv: np.ndarray, xy: np.ndarray, z: np.ndarray, shift: np.ndarray):
        ...
        xy_proj = xy / denom
        denom2 = np.square(xy_proj).sum()
        if not np.isfinite(denom2) or denom2 < 1e-12:
            return np.zeros(uv.size, dtype=np.float64)
        f = (xy_proj * uv).sum() / denom2
    ...
    try:
        ...
        # least_squares 初始点非有限或求解失败时的兜底
        print(f"[solve_optimal_focal_shift] least_squares failed: {e}, fallback to shift=0")
        optim_shift = np.float32(0.0)

    xy_proj = xy / (z + optim_shift)[:, None]
    denom2 = np.square(xy_proj).sum()
    if not np.isfinite(denom2) or denom2 < 1e-12:
        optim_focal = np.float32(1.0)
    else:
        optim_focal = (xy_proj * uv).sum() / denom2
        if not np.isfinite(optim_focal):
            optim_focal = np.float32(1.0)
...
def solve_optimal_shift(uv: np.ndarray, xyz: np.ndarray, focal: float):
    ...
    try:
        ...
        print(f"[solve_optimal_shift] least_squares failed: {e}, fallback to shift=0")
        optim_shift = np.float32(0.0)
