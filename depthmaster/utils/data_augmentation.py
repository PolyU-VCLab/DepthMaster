...
def warp_perspective(
    ...
    - `transform` is the matrix that transforms hodepthmasterneous pixel coordinates of source image to those of target image, i.e., `p_tgt = transform @ p_src`.
    """
    ...
    if 'dof' in augmentations:
        ...
        if rng.uniform() < 0.5:
            ...
            if finite_mask.sum() > 0:
                disp_min, disp_max = float(disp[finite_mask].min()), float(disp[finite_mask].max())
                if np.isfinite(disp_min) and np.isfinite(disp_max) and disp_max > disp_min and (disp_max - disp_min) < 1e10:
