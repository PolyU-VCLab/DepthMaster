...
from ..train.dataset_readers import get_reader
...
class EvalDataLoaderPipeline:
    ...
    def _load_instance(self, idx):
        ...
        if self.data_format is not None:
            # 使用 dataset_readers 注册表读取自定义格式数据集
            reader = get_reader(self.data_format)
            reader_instance = {'path': path}
            result = reader(reader_instance, read_image)
            if result is None:
                return None
            depth = result['depth']
            ...
            instance['intrinsics'] = result['intrinsics']
        else:
            # 默认 DepthMaster 统一格式: image.jpg + depth.png + meta.json
    ...
    def _process_instance(self, instance: dict):
        ...
        # do hodepthmasterneous transformation with the rotation and intrinsics
