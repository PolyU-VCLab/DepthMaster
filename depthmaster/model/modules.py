...
class DINOv2Encoder(nn.Module):
    ...
    def __init__(self, backbone: str, intermediate_layers: Union[int, List[int]], dim_out: int, use_face_embed: bool = False, **deprecated_kwargs):
        ...
        self.backbone = self.hub_loader(pretrained=False, use_face_embed=use_face_embed)
    ...
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False, camera_type: str = None, return_intermediate_features: bool = False, condition_tokens: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        features = self.backbone.get_intermediate_layers(image_14, n=self.intermediate_layers, return_class_token=True, camera_type=camera_type, condition_tokens=condition_tokens)
        ...
        # 求和得到最终特征（与原逻辑一致）
        x = torch.stack(projected_features, dim=1).sum(dim=1)

        if return_intermediate_features:
            # 返回投影后的4层中间特征列表，每层形状为 (B*V, C_out, H, W)
            if return_class_token:
                return x, features[-1][1], projected_features
            else:
...
class ConvStack(nn.Module):
    ...
    def forward(self, in_features: List[torch.Tensor], border_padding: int = 0):
        """
        Args:
            in_features: 各层级的输入特征列表
        ...
        for i in range(len(self.res_blocks)):
            ...
            if i < len(self.res_blocks) - 1:
                if border_padding > 0:
                    # 上采样前做 replicate padding，让边界卷积有合理上下文
                    x = F.pad(x, [border_padding] * 4, mode='replicate')
                x = self.resamplers[i](x)
                if border_padding > 0:
                    # 上采样后 crop 回正确尺寸（padding 被放大了 scale_factor=2 倍）
                    p = border_padding * 2
                    x = x[:, :, p:-p, p:-p]
        return out_features
